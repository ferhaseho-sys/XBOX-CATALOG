...
export function useScraper() {
  ...
  // Cargar juegos REALES desde la API de Atlas (reemplaza los mock)
  useEffect(() => {
    const ATLAS_API = (import.meta as any).env?.VITE_ATLAS_API || 'http://127.0.0.1:8000';
    let cancelled = false;
    const mapGame = (p: any) => ({
      id: p.product_id, productId: p.product_id, title: p.title, region: 'US',
    ...
    // Carga liviana SOLO para la vista Tabla y los filtros. La vista principal
    // (Tarjetas / GameCards) navega TODO el catálogo con paginación server-side,
    // así que acá alcanza con una tanda chica (evita flasheo y consumo de IO).
    const loadOnce = async () => {
      try {
        const batch = await fetch(`${ATLAS_API}/api/games?limit=500`).then(r => (r.ok ? r.json() : []));
        if (!cancelled && Array.isArray(batch) && batch.length) {
          setGames(batch.map(mapGame));
        }
      } catch { /* noop */ }
    };
    loadOnce();
  ...
  // "Start" ahora dispara el ANÁLISIS COMPLETO del catálogo (sitemaps -> precios),
  // no un scraping. Es una acción ocasional; se sigue el progreso por la API.
  const startScraping = useCallback(async (_config: ScrapingConfig) => {
    const ATLAS_API = (import.meta as any).env?.VITE_ATLAS_API || 'http://127.0.0.1:8000';
    ...
    const poll = async () => {
      ...
      try {
        ...
        setProgress(prev => ({ ...prev, isActive: !!s.running, status: labels[s.phase] || s.detail || s.phase, currentRegion: s.phase }));
